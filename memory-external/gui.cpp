...
// Remove the separate DirectX variables since we'll use the main renderer's context
...
bool InitImGui(HWND hwnd, ID3D11Device* device, ID3D11DeviceContext* deviceContext)
{
    ...
    style.Alpha = 0.95f; // Slight transparency
...
{
    if (gui_initialized) {
        // ImGui will handle resize automatically through the backends
    }
}
...
// Remove GuiThread function since we're not using a separate thread anymore